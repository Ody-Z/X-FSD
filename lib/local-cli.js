...
async function callLocalGeminiCliBridge({
  ...
  const result = await callLocalBridgeWithUsage({
    systemPrompt,
...
async function callLocalGeminiCliBridgeWithUsage({
  systemPrompt,
  userPrompt,
  tweetText,
  context,
  ...
  return callLocalBridgeWithUsage({
    systemPrompt,
    userPrompt: userPrompt || null,
    tweetText,
    context,
...
async function callLocalClaudeCodeBridgeWithUsage({
  systemPrompt,
  userPrompt,
  tweetText,
  context,
...
async function callLocalBridgeWithUsage({
  systemPrompt,
  userPrompt,
  tweetText,
  context,
  ...
  try {
    const res = await fetch(`${bridgeOrigin}/generate-reply`, {
      ...
      body: JSON.stringify({ systemPrompt, userPrompt, tweetText, context, model, requestId, timeoutMs, mode, phase })
    });

    const data = await res.json().catch(() => ({}));
    const message = `[XGA][bridge-client][${requestId}] HTTP ${res.status} in ${formatDuration(nowMs() - startedAt)}`;
    if (data.tokenUsage) {
      console.log(message, { tokenUsage: data.tokenUsage });
    } else {
      console.log(message);
    }
