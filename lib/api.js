...
function buildSystemPrompt(tone, toneData) {
  ...
  if (comparisons.length > 0) {
    ...
    for (const c of comparisons.slice(-MAX_COMPARISON_EXAMPLES)) {
      system += `\nOriginal post: ${truncateText(c.originalPost, MAX_COMPARISON_TEXT_LENGTH)}\nAI Generated: ${truncateText(c.aiGenerated, MAX_COMPARISON_TEXT_LENGTH)}\nUser Final: ${truncateText(c.userFinal, MAX_COMPARISON_TEXT_LENGTH)}\n---`;
    }
  }
...
function buildUserMessage(tweetText, context) {
  ...
  if (threadTweets.length > 1) {
    ...
    threadTweets.slice(-(MAX_THREAD_CONTEXT_TWEETS + 1), -1).forEach((t, i) => { msg += `[${i + 1}] ${t}\n`; });
    msg += '\n';
  }
