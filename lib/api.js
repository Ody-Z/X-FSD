...
const AUTO_STRATEGY_CONFIG = {
  case_data: {
    label: 'Case / Data',
    baseTone: 'smart',
    instruction: 'Add one specific case, counterexample, or number the post did not mention. Use only facts present in the supplied context or voice references, or verified by an available search tool in the current run. Never invent evidence.'
  },
  first_hand: {
    label: 'First-hand',
    baseTone: 'supportive',
    instruction: 'Use a real first-hand experience to support, complicate, or challenge the post. Only use experiences explicitly present in the supplied context, voice profile, writing samples, or past edits. Never fabricate an "I" story.'
  },
  boundary_condition: {
    label: 'Boundary Condition',
    baseTone: 'smart',
    instruction: 'Identify the condition under which the post is true, false, or materially different. Name the missing variable or limit precisely instead of disagreeing for effect.'
  }
};
...
const MAX_LINK_TEASER_TEXT_LENGTH = 160;
...
function buildSystemPrompt(tone, toneData, options = {}) {
  ...
  let system = `You generate high-value X (Twitter) replies in the user's voice.\n\n${buildVoiceGuideSection(options.voiceProfile)}\n\nTone instruction:\n${prompt}\n\nPriority:\n- The voice guide has final say over wording, capitalization, punctuation, rhythm, and attitude. The target post's style and the tone instruction must not overwrite it.\n- Add new information or judgment. Do not merely agree, praise, paraphrase, summarize, or ask an empty engagement question.\n- Prefer one value move: add a specific case or data point; add a real first-hand judgment; or identify a precise boundary condition.\n- Never invent a statistic, case, source, personal experience, or product result. A factual example may come from the supplied context, voice references, or an available search tool used in the current run. If none can support it, use a boundary condition or write a modest reaction without fabricated evidence.\n- State the contribution directly. Do not lead with throat-clearing such as "great point", "exactly", or "this".\n- When a quoted post is provided, use it as core context before replying.\n- When a linked article preview is provided, use its title and excerpt as attachment context. Do not claim to have read the full article unless full article text is explicitly provided.\n- Keep it compact. Use line breaks only when they fit the user's voice.\n- No hashtags unless the voice guide permits them.`;
...
function buildAdaptiveSystemPrompt({
  ...
  const phaseRule = quickMode
    ? 'Optimize for speed. Return a usable first draft, not a perfect one. Keep the reply under 220 characters. Prefer 1 compact thought. Never exceed 2 sentences.'
    : 'This is a quality pass. Make the contribution more precise, not more polished or verbose. Keep the reply under 280 characters. Use the style references when they help. Never exceed 2 sentences.';

  let system = [
    'Write high-value X reply drafts.',
    buildVoiceGuideSection(voiceProfile),
    'The voice guide is the top-level writing constraint. Preserve its capitalization, punctuation, rhythm, attitude, and prohibited patterns even when the target post sounds different.',
    'First decide whether you can add genuine value. If yes, pick exactly one value move and write the reply in the same pass.',
    `Value moves: ${strategySummary}`,
    'A ready reply must contribute something the post did not already say. Generic agreement, praise, paraphrase, summary, an obvious question, and engagement bait all fail this gate.',
    'Use only facts found in the supplied context or voice material, or verified by an available search tool in the current run. First-hand experiences must appear explicitly in the voice profile, writing samples, style references, or post context. Never invent a number, case, source, personal story, credential, relationship, or product result.',
    'If case_data and first_hand lack support, use boundary_condition only when a real missing variable or limit is visible. Otherwise skip the post.',
    'State the added value immediately. Do not open with "great point", "exactly", "well said", or similar throat-clearing.',
    'Skip low-signal repost shells, posts that are too thin for one of the three value moves, or risky/sensitive posts where an auto-reply feels wrong.',
    ...
    '{"status":"ready|skipped","strategyType":"case_data|first_hand|boundary_condition|null","baseTone":"supportive|smart|null","reply":"string","reason":"string"}',
    ...
    'Do not imitate the target post at the expense of the user voice. Do not use hashtags unless the voice guide permits them.',
    phaseRule
  ].join('\n');
...
function isShortLinkTeaserText(rawText) {
  const original = typeof rawText === 'string' ? rawText : '';
  const nonUrlLines = original
    .split(/\n+/)
    .map((line) => normalizeWhitespace(line.replace(/https?:\/\/\S+/gi, '')))
    .filter(Boolean);
  const text = normalizeWhitespace(original.replace(/https?:\/\/\S+/gi, ''));
  if (!text || text.length > MAX_LINK_TEASER_TEXT_LENGTH || nonUrlLines.length > 1) return false;

  const words = text.match(/[\p{L}\p{N}]+/gu) || [];
  if (words.length > 24) return false;

  return /:\s*$/.test(text) ||
    /\b(subscribe|newsletter|read|watch|listen|check\s+out|sign\s+up|pre-?order|download|join|apply|register|link\s+in|details?\s+here|full\s+(post|article|episode|video|interview)|more\s+here)\b/i.test(text);
}

function isSubscribeCardPromptText(rawText) {
  const text = normalizeWhitespace(rawText);
  return /^(click\s+to\s+)?subscribe\s+to\s+@?[\p{L}\p{N}_-]+$/iu.test(text);
}
...
function guessStrategyForTone(tone, fallback = 'first_hand') {
  switch (tone) {
    case 'funny':
      return 'first_hand';
    case 'question':
    ...
    case 'supportive':
      return 'first_hand';
    case 'enhance':
      return AUTO_STRATEGY_CONFIG[fallback] ? fallback : 'boundary_condition';
    default:
      return 'boundary_condition';
  }
}
...
function detectAutoDraftSkipReason(tweetText, context = null) {
  ...
  if (text && !quotedText && isSubscribeCardPromptText(tweetText)) {
    return 'Subscription card skipped by rule.';
  }

  if (linkedArticleText && text && !quotedText && !hasMediaContext && isShortLinkTeaserText(tweetText)) {
    return 'Short link teaser skipped by rule.';
  }
