...
function buildAdaptiveSystemPrompt({ phase = DRAFT_PHASE_QUICK, toneDataByTone = null } = {}) {
  ...
  const phaseRule = quickMode
    ? 'Optimize for speed. Return a usable first draft, not a perfect one. Keep the reply under 220 characters. Use 1 to 3 short lines max.'
    : 'This is a quality pass. Be sharper and more polished than the quick draft. Keep the reply under 280 characters. Use the style references when they help.';
...
function normalizeAdaptiveDraftResult(payload) {
  ...
  return {
    ...
    strategyType,
    baseTone,
    text: truncateText(reply, MAX_DRAFT_TEXT_LENGTH),
    reason: ''
  };
}
