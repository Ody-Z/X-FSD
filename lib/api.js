...
const ODY_STYLE_LINES = [
  'Use very light punctuation overall.',
  'Do not use em dashes, en dashes, or dash-style asides.',
  'If the reply is one line, use no punctuation unless a handle or link requires it.',
...
function buildOdyVoiceSection() {
  return [
    'Voice guide:',
    ...ODY_PERSONA_LINES.map((line) => `- ${line}`),
    ...ODY_STYLE_LINES.map((line) => `- ${line}`)
  ].join('\n');
}
...
function buildAdaptiveSystemPrompt({ phase = DRAFT_PHASE_QUICK, toneDataByTone = null } = {}) {
  ...
  if (phase === DRAFT_PHASE_FULL) {
    system += buildAdaptiveComparisonSection(toneDataByTone);
  }
...
function buildAdaptiveDraftPrompt({ tweetText, context, phase = DRAFT_PHASE_QUICK, toneDataByTone = null } = {}) {
  return {
    systemPrompt: buildAdaptiveSystemPrompt({ phase, toneDataByTone }),
    userMessage: buildUserMessage(tweetText, context)
  };
}
...
function buildManualDraftPrompt({
  ...
  if (tone === 'enhance') {
    const systemPrompt = buildSystemPrompt('enhance', toneData);
    return {
      systemPrompt,
...
function detectAutoDraftSkipReason(tweetText) {
  const text = normalizeWhitespace(tweetText);
  if (!text) return 'Post has no usable text.';

  const linkless = text.replace(/https?:\/\/\S+/gi, '').trim();
  const alnum = linkless.replace(/[^\p{L}\p{N}]+/gu, '');
  if (alnum.length < 12) return 'Post is too low-signal for an auto-reply.';

  if (/^(rt\s+@|repost\b|quote tweet\b)/i.test(text)) {
  ...
  if (/(rip|rest in peace|passed away|condolences|earthquake|shooting|bombing|massacre|victims?|obituary|memorial)/i.test(text)) {
...
async function generateReply(tweetText, tone, toneData, settings, context) {
  ...
  if (settings.activeModel === GEMINI_MODEL) {
    ...
    return (await callGemini(settings.geminiApiKey, systemPrompt, userMessage)).trim();
  }

  if (settings.activeModel === 'kimi-k2.5') {
    if (!settings.moonshotApiKey) throw new Error('Moonshot API key not set');
    return (await callKimi(settings.moonshotApiKey, systemPrompt, userMessage, settings.moonshotEndpoint)).trim();
  }

  if (settings.activeModel === 'claude-haiku') {
    if (!settings.anthropicApiKey) throw new Error('Anthropic API key not set');
    return (await callClaude(settings.anthropicApiKey, systemPrompt, userMessage)).trim();
  }
