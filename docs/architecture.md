...
- `background.js` handles extension messages, reads cached settings, builds prompts, routes provider calls, saves style-learning comparisons, and opens post tabs.
...
7. Sending opens X's reply composer, inserts the final text, clicks the send button, stores sent metadata, and saves an edit comparison for future full-quality prompts.
